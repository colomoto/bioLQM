...
public final class AvatarUtils {
	...
	public static int[] getFreeChildren(int nstates) {
		...
		for(int i=0; i<nstates; i++) children[i]=-1;
		return children;
	}
	...
	public static byte[] getFreeState(int ncomponents) {
		...
		for(int i=0; i<ncomponents; i++) array[i]=-1;
		return array;
	}
	...
	public static int[] getIdentityChildren(int nstates) {
		int[] children = new int[nstates];
		for(int i=0; i<nstates; i++) children[i]=i;
		return children;
	}
	 ...
	 * @return the array with the initialize 'node' component (e.g. [-1,-1,..,-1,p,-1,..-1]=
	 */
	public static int[] getChildrenWithSingleNode(int nstates, int node) {
		int[] children = new int[nstates];
		for(int i=0; i<nstates; i++) children[i]=node;
		return children;
	}
	...
	public static String toString(List<byte[]> list) {
		String result = "{";
		for(byte[] vec : list) result+=toString(vec);
		return result+"}";
	}
	...
	public static String toString(byte[] vector) {
		...
		for(int i=1, l=vector.length; i<l; i++) result+=","+vector[i];
		return result+"]";
	}
	...
	public static String toString(double[] vector) {
		if(vector.length==0) return "[]";
		String result = "["+vector[0];
		for(int i=1, l=vector.length; i<l; i++) result+=","+vector[i];//String.format("%.2f",vector[i]);
		return result+"]";
	}
	...
	public static String toString(int[][] matrix) {
		...
		for(int i=0, l=matrix.length; i<l; i++) result.append(toString(matrix[i])+"\n");
	...
	public static int[] toArray(List<Integer> list) {
		...
		for(int i=0, l=list.size(); i<l; i++) array[i]=list.get(i);
		return array;
	}
	...
	public static byte[] toByteArray(int[] vector) {
		...
		for(int i=0, l=vector.length; i<l; i++) array[i]=(byte)vector[i];
		return array;
	}
}
